...
private:
  ...
  // Save the initial x and the final x of the track after it is t0-tagged (for a comparison to the initial reconstructed coordinates).
  double _rc_x_start_t0_tagged;
  double _rc_x_end_t0_tagged;
  
  // Declare vectors of doubles for the amount of PE deposited in the truth and hypo flashes.
  std::vector< double > hypo_flash_pe;
  std::vector< double > data_flash_pe;

  // Declare an iterator for the event that I am running over.
  int event_iter;
  ...
  // functions to be used for my specific flash-matching purposes.
  void ExtendQClusterStartingWithTVectorPoints( flashana::QCluster_t& qcluster, const std::vector< TVector3 >& sorted_trk, double extension, bool extend_start, bool extend_end);
  bool isNearActiveVolumeEdge( ::geoalgo::Vector pt, double d );
  flashana::Flash_t MakeDataFlash( const recob::OpFlash* flash );
  //void ApplyCorrectionstoDataFlash( flashana::Flash_t data_flash, double light_yield );
...
void T0RecoAnodeCathodePiercingAna::beginJob()
{
  ...
  _tree->Branch("_rc_x_start_t0_tagged",&_rc_x_start_t0_tagged,"rc_x_start_t0_tagged/D");
  _tree->Branch("_rc_x_end_t0_tagged",&_rc_x_end_t0_tagged,"rc_x_end_t0_tagged/D");
  ...
  // Declare vectors for the data and hypo flash PE.
  _tree->Branch("hypo_flash_pe", &hypo_flash_pe);
  _tree->Branch("data_flash_pe", &data_flash_pe);
...
void T0RecoAnodeCathodePiercingAna::analyze(art::Event const & e)
{
  ...
  std::cout << "Currently iterating over event #" << event_iter << "." << std::endl;
  // Increment 'event_iter'.
  ++event_iter;

  // Reset the FlashMatchManager.
  _mgr.Reset();
 
  // Declare the geometry tool here.
  ...
  for (size_t i=0; i < track_h->size(); i++){
    ...
    if (T0_v.size() == 1){
      ...
      // These do not include the front-piercing or back-piercing functions for tracks that pierce those faces, but I will be abe to identify them anyway from
      // their trajectories.
      ...
      // Continue if the track pierced the anode.
      if ( _cathode == 1 ) continue;

      // reconstructed time comes from T0 object
      _rc_time = t0->Time();

      // Calculate an original qcluster for matching the flash to the reconstructed track information.
      flashana::QCluster_t original_qcluster;
      ExtendQClusterStartingWithTVectorPoints(original_qcluster, sorted_trk, 10000., true, true);

      // Correct all of x-coordinates with the reconstructed t0, giving the track full TPC coordinate information.
      for ( size_t pt_iter = 0; pt_iter < sorted_trk.size(); ++pt_iter ) {

	// Correct the x-coordinate of each point using the t0 and the drift velocity.
	sorted_trk.at( pt_iter )[0] -= fDriftVelocity*_rc_time;

      }

      // Fill '_rc_x_start_t0_tagged' and '_rc_x_end_t0_tagged' with the starting and ending points on 'sorted_trk'.
      _rc_x_start_t0_tagged = sorted_trk.at( 0 )[0];
      _rc_x_end_t0_tagged   = sorted_trk.at( sorted_trk.size() - 1 )[0];

      //**************** The code that I've added ********************//
      // Clear 'hypo_flash_pe' and 'data_flash_pe'
      hypo_flash_pe.clear();
      data_flash_pe.clear();
      
      // Resize both of these vectors for the number of PMTs.
      hypo_flash_pe.resize(32, 0.);
      data_flash_pe.resize(32, 0.);

      // Now that the track is t0-tagged, generate a qcluster object for the flash.
      // Declare a new qcluster, which will be made from the points of 'sorted_trk' and then extended  into the cryostat.
      flashana::QCluster_t qcluster;
      ExtendQClusterStartingWithTVectorPoints(qcluster, sorted_trk, 10000., true, true);

      // Generate a flash hypothesis for this track using larlite machinery.
      // Declare a flash hypothesis object.
      flashana::Flash_t flash_hypothesis;
      flash_hypothesis.pe_v.resize(32); // hardcode this as 32 for now.
      ((flashana::PhotonLibHypothesis*)(_mgr.GetAlgo(flashana::kFlashHypothesis)))->FillEstimate(qcluster,flash_hypothesis);

      flashana::Flash_t original_flash_hypothesis;
      original_flash_hypothesis.pe_v.resize(32); // hardcode this as 32 for now.                                                                                                                         
      ((flashana::PhotonLibHypothesis*)(_mgr.GetAlgo(flashana::kFlashHypothesis)))->FillEstimate(original_qcluster,original_flash_hypothesis);

      // Apply the light yield and cosmic discriminator corrections here as well.
      //ApplyCorrectionstoDataFlash( flash_hypothesis, 40000.);
      for ( size_t i = 0; i < flash_hypothesis.pe_v.size(); ++i ) {                                                                                                                                        
	
	original_flash_hypothesis.pe_v.at( i ) *= 40000.;
	
	if ( original_flash_hypothesis.pe_v.at( i ) < 60.0 )
          original_flash_hypothesis.pe_v.at( i ) *= 0.424;
        else if( original_flash_hypothesis.pe_v.at( i ) > 60.0 )
          original_flash_hypothesis.pe_v.at( i ) *= 0.354;
      
      }

      // Look at the reconstructed points for the point matched to the flash BEFORE t0-tagging the track.
      double matched_TPC_point_x_not_t0_tagged = ((flashana::QLLMatch*)(_mgr.GetAlgo(flashana::kFlashMatch)))->Match(original_qcluster, original_flash_hypothesis).tpc_point.x;
      double matched_TPC_point_y_not_t0_tagged = ((flashana::QLLMatch*)(_mgr.GetAlgo(flashana::kFlashMatch)))->Match(original_qcluster, original_flash_hypothesis).tpc_point.y;
      double matched_TPC_point_z_not_t0_tagged = ((flashana::QLLMatch*)(_mgr.GetAlgo(flashana::kFlashMatch)))->Match(original_qcluster, original_flash_hypothesis).tpc_point.z;
	...
	// Convert this opflash into a data flash.
	flashana::Flash_t data_flash = MakeDataFlash( flash );
	
	_pe_flash = flash->TotalPE();
	_t_match  = flash->Time();
	_dt_flash = fabs( flash->Time() - _rc_time );

      // Loop through 'hypo_flash_pe' and 'data_flash_pe' and fill them with the number of PEs in each of the opdets.
      for ( size_t opchan_iter = 0; opchan_iter < flash_hypothesis.pe_v.size(); ++opchan_iter ) {

	hypo_flash_pe.at( geo->OpDetFromOpChannel( opchan_iter ) ) = flash_hypothesis.pe_v.at( geo->OpDetFromOpChannel( opchan_iter ) );
	data_flash_pe.at( geo->OpDetFromOpChannel( opchan_iter ) ) = data_flash.pe_v.at( geo->OpDetFromOpChannel( opchan_iter ) );

	// Make sure that the flashes look good:
	std::cout << "The amount of light in OpDet #" << geo->OpDetFromOpChannel( opchan_iter ) << " for the hypo flash = " << hypo_flash_pe.at( geo->OpDetFromOpChannel( opchan_iter ) ) << std::endl;
	std::cout << "The amount of light in OpDet #" << geo->OpDetFromOpChannel( opchan_iter ) << " for the data flash = " << data_flash_pe.at( geo->OpDetFromOpChannel( opchan_iter ) ) << std::endl;

      }

      // Print both values out before saving them to a tree.
      std::cout << "The x-coordinate for the flash that was minimized (but not t0-tagged) = " << matched_TPC_point_x_not_t0_tagged << "." << std::endl;
      std::cout << "The y-coordinate for the flash that was minimized (but not t0-tagged) = " << matched_TPC_point_y_not_t0_tagged << "." << std::endl;
      std::cout << "The z-coordinate for the flash that was minimized (but not t0-tagged) = " << matched_TPC_point_z_not_t0_tagged << "." << std::endl;
...
// Declare the function that will generate a qcluster and extend it using the 'LightPath' functionality.
void T0RecoAnodeCathodePiercingAna::ExtendQClusterStartingWithTVectorPoints( flashana::QCluster_t& qcluster, const std::vector< TVector3 >& sorted_trk, double extension, bool extend_start, bool extend_end ) {

  // Return an empty qcluster if the number of entries in the std::vector are less than 2
  if ( sorted_trk.size() < 2 ) return;

  // Declare the two poinnts that will be used in the function.
  ...
  // Add body
  //
  for ( size_t step_idx = 0; step_idx < sorted_trk.size() - 1; ++step_idx ) {

    pt0[0] = sorted_trk.at( step_idx )[0];
    pt0[1] = sorted_trk.at( step_idx )[1];
    pt0[2] = sorted_trk.at( step_idx )[2];

    pt1[0] = sorted_trk.at( step_idx + 1 )[0];
    pt1[1] = sorted_trk.at( step_idx + 1 )[1];
    pt1[2] = sorted_trk.at( step_idx + 1 )[2];

    lightpath.QCluster( pt0, pt1, qcluster );
  }

  // Initialize the first point on the track's trajectory.
  pt0[0] = sorted_trk.at( 0 )[0]; pt0[1] = sorted_trk.at( 0 )[1]; pt0[2] = sorted_trk.at( 0 )[2];
  ...
  if ( isNearActiveVolumeEdge(pt0, 10.0) && extend_start ) {
    ...
    pt1[0] = sorted_trk.at( 1 )[0]; pt1[1] = sorted_trk.at( 1 )[1]; pt1[2] = sorted_trk.at( 1 )[2];

    // Turn 'pt1' into a direction vector (not normalized).
    pt1    = pt0 - pt1;
    pt0    = pt0 + pt1.Dir()*extension;
    pt1[0] = sorted_trk.at( 0 )[0]; pt1[1] = sorted_trk.at( 0 )[1]; pt1[2] = sorted_trk.at( 0 )[2];

    // Expand the existing qcluster using the points 0 and 1.
    lightpath.QCluster( pt0, pt1, qcluster );

  }

  // Initialize the last point on the track's trajectory.
  pt0[0] = sorted_trk.at( sorted_trk.size() - 1 )[0]; pt0[1] = sorted_trk.at( sorted_trk.size() - 1)[1]; pt0[2] = sorted_trk.at( sorted_trk.size() - 1)[2];

  if ( isNearActiveVolumeEdge(pt0, 10.0) && extend_end) {

    pt1[0] = sorted_trk.at( sorted_trk.size() - 2)[0]; pt1[1]= sorted_trk.at( sorted_trk.size() - 2)[1]; pt1[2]= sorted_trk.at( sorted_trk.size() - 2)[2];

    pt1 = pt0 - pt1;
    pt0 = pt0 + pt1.Dir()*extension;
    pt1[0] = sorted_trk.at( sorted_trk.size() - 1 )[0]; pt1[0] = sorted_trk.at( sorted_trk.size() - 1 )[0];
...
// Define a function that will check if a point is located near the edge of the active detector volume.
bool T0RecoAnodeCathodePiercingAna::isNearActiveVolumeEdge( ::geoalgo::Vector pt, double d ) {

  // check if the point are located near the (hard-coded) boundaries of the detector. return true if they are.
...
flashana::Flash_t T0RecoAnodeCathodePiercingAna::MakeDataFlash( const recob::OpFlash* flash ) {

  // Declare a geometry object used to make an OpDet from an OpChannel.
  const larutil::Geometry* geo = ::larutil::Geometry::GetME();

  // Declare a data flash object that will be filled with the important information.
  flashana::Flash_t f;
  f.x = f.x_err = 0;
  f.y = flash->YCenter();
  f.z = flash->ZCenter();
...
// Declare a function to use Kazu's minimized but with my chi2 calculation function.
void T0RecoAnodeCathodePiercingAna::ApplyCorrectionstoDataFlash( flashana::Flash_t data_flash, double light_yield ) {

  // Loop through the 'data_flash' and multiply by the light yield first and then by the cosmic discrimimator corrections.
  for ( size_t i = 0; i < data_flash.pe_v.size(); ++i ) {

    // Multiply by the light yield at each point in this object.
    data_flash.pe_v.at( i ) *= light_yield;
    
    // Next, multiply by a correction for the cosmic discriminator based on the PE value in the hypothesis flash.
    if ( data_flash.pe_v.at( i ) < 60.0 ) 
      data_flash.pe_v.at( i ) *= 0.424;

    else
      data_flash.pe_v.at( i ) *= 0.354;
